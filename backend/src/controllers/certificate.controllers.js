...
const createCertificate = asyncHandler(async (req, res) => {
    ...
    if (!fullname || !description || !endDate || !startDate) throw new ApiError(400, "Fill all the required inputs");

    const certificateId = getCertId(startDate);
    if (!certificateId) throw new ApiError(500, "Internal error while generating certificate ID");

    const certificate = await Certificate.create({ certificateId, fullname, description, endDate, startDate });

    if (!certificate) throw new ApiError(500, "Internal error while creating certificate");
